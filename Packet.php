...
public static function writeNetworkText(
    string $text,
    int $mode = 0,
    array $substitutions = []
): string {
    $out = chr($mode) . self::writeString($text);

    if ($mode !== 0) {
        $out .= chr(count($substitutions));
        foreach ($substitutions as $nt) {
            $out .= $nt; // already encoded NetworkText
        }
    }

    return $out;
}
public static function buildPacket(int $id, string $payload): string
{
    $length = strlen($payload) + 3;
